...
#
# It lives WITH the account it tests.  The suite and the fixture are one thing:
# a dictionary changed here is a dictionary this suite must still agree with,
# and when they sat in different repositories the pairing was a convention
# nobody could enforce -- mvx carried the suite and nothing ever ran it.
#
# The cost is that a clone of the demo account gets a tests/ directory it does
# not need.  That is harmless rather than merely tolerable: mv_git leaves a
# plain subdirectory on disk untouched at checkout, exactly as it does README
# and .gitmodules, so it is never read as an MV file.
#
#   PLATFORM = mvx | udt | uv | jbase        (default mvx)
#   MVX      = the runtime / platform binary (mvx | udt | uv | jsh)
#   MVXGIT   = the git driver                (mvx-git | udt-git | uv-git | jb-git)
...
# ti NAME EXPECTED ACTUAL — substring assertion, case-insensitive.  Only the
# record counts need this: "12 record(s)" on MVX, "12 Records counted" on
# jBASE.  The capital R is the whole difference, and asserting the platform's
# exact wording would test the wording rather than the count.
ti()   { _a=$(printf '%s' "$3" | tr 'A-Z' 'a-z'); _e=$(printf '%s' "$2" | tr 'A-Z' 'a-z')
         case "$_a" in *"$_e"*) ok "$1";; *) bad "$1" "$2" "$3";; esac; }
...
if [ "$PLATFORM" = mvx ]; then
  ...
  # jBASE's $MVX is jsh -- its own SHELL, not a TCL monitor -- so it reads a
  # sentence from stdin and leaves at EOF.  There is no QUIT verb to send it:
  # feeding one appends "QUIT: No such file or directory" to every answer,
  # which is a substring failure in whichever assertion reads it.
  V() { ( cd "$A" && printf '%s\n' "$*" | "$MVX" ) 2>&1; }
else
...
ti "clients"   "12 record"  "$(V COUNT CLIENTS)"
ti "inventory" "15 record"  "$(V COUNT INVENTORY)"
ti "orders"    "20 record"  "$(V COUNT ORDERS)"
ti "staff"     "8 record"   "$(V COUNT STAFF)"
ti "states"    "8 record"   "$(V COUNT STATES)"
